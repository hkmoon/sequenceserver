var TreeCompare = function() {
    ...
    function changeTreeSettings(settingsIn) {
        ...
        settings.checkbox = getSetting(settingsIn.checkbox,settings.checkbox);
    ...
    function convertTree(s) { //s is newick file format
        ...
        function updateGroupColor(subtree, color) {
          if(subtree.children) {
            subtree.children.forEach(function (child) {
              child.specifiedBranchColor = color;
              updateGroupColor(child, color);
            });
          }
        }

        groups.forEach(function (node) {
          updateGroupColor(node, node.specifiedBranchColor);
          // console.log(node.specifiedBranchColor)
          // console.log(rgb2hex(node.specifiedBranchColor))

          // The triangle color is filled with branchColor
          node.branchColor = node.specifiedBranchColor;
          node.specifiedBranchColor = null;
        })
    ...
    function applyScaleText(scaleText, zoomScale, root) {
        if (root.children || root._children) {
            ...
            scaleText.text(text + ' (Substitutions per site)');
        }
    }
    ...
    function update(source, treeData, duration, treeToggle) {
        ...
        if( settings.checkbox ) {
          nodeEnter.each(function (d) {
            if (!d.children && !d._children) { //for leaf nodes

              // console.log(d.name)

              var found = d.name.match(/\"(.+)\"$/g)

              if(found) {
                var transcriptome = found[0].replace(/^"(.*)"$/, '$1')

                var checkBox = new d3CheckBox()
                checkBox.size(10).x(10).y(-5).markStrokeWidth(2).boxStrokeWidth(2).checked(false).clickEvent(function () {
                  var checkStaus = checkBox.checked()

                  $("input.box").each(function() {
                    // console.log($(this).attr('id'))
                    if(this.id.indexOf(transcriptome) > -1){
                      $(this).prop('checked', checkStaus);
                      $(this).trigger('database_type_changed', $('.databases input:checked').data('type'));
                    }
                  });

                  $.cookie(transcriptome, checkStaus, {
                    path: '/',
                    expires: 365
                  });
                })
                ...
                var mycookie = $.cookie(transcriptome);
                if (mycookie && mycookie === "true") {
                  checkBox.checked(true)
                }

                if (db_param && db_param.indexOf(transcriptome) > -1)
                  checkBox.checked(true)

                if (org_param && transcriptome.indexOf(org_param) > -1)
                  checkBox.checked(true)
        ...
        nodeUpdate.select("text")
            ...
            .attr("x", function(d) {
                if ((!d.children || d._children) && treeName === trees[treeIdx].name && settings.mirrorRightTree){ //mirrored right tree
                    ...
                    return settings.checkbox ? 28 : 13;
                } else { //internal nodes
